...
func (t *tectonic) Generate(dependencies map[asset.Asset]*asset.State) (*asset.State, error) {
	...
	templateData := &tectonicTemplateData{
		IngressCaCert:                          base64.StdEncoding.EncodeToString([]byte(strings.Join([]string{string(ingressContents[certIndex].Data), string(ingressContents[keyIndex].Data), string(dependencies[t.kubeCA].Contents[certIndex].Data)}, ""))),
